...
axiosInstance.interceptors.response.use(
  ...
  (error) => {
    ...
    // Only force-logout on 401 from non-logout endpoints.
    // Logout itself may 401 if token is already expired — that's fine,
    // the useLogout hook handles cleanup via onSettled/onSuccess.
    if (error?.response?.status === 401 && !isLogoutUrl) {
