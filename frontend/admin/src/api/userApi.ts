...
export const userApi = {
  ...
  requestReaderEmailOtp: async (email: string) => {
    const response = await axiosInstance.post('/private/v1/users/email-verification/request', { email });
    return response.data;
  },

  verifyReaderEmailOtp: async (email: string, otp: string) => {
    const response = await axiosInstance.post('/private/v1/users/email-verification/verify', { email, otp });
