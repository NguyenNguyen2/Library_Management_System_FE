...
const ForgotPassword = () => {
  ...
  const { secondsLeft, startCooldown, stopCooldown } = useOtpCooldown(
    STORAGES.OTP_DEADLINE,
    60
  );
  ...
  return (
    <div className="flex flex-col items-center w-full">
      ...
      >
        ...
        {/* Cooldown timer */}
        {step === FORGOT_PW_STEP.OTP && (
          <Flex align="center" gap={16} className={cn('mt-3')}>
            <Typography.Text>
              {t(getKey('resend_available')) +
                (secondsLeft > 0
                  ? ` ${t(getKey('in'))}: ${formatSecondsToMMSS(secondsLeft)}`
                  : '')}
            </Typography.Text>
            <Button
          ...
          <>
            ...
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: t(getKey('confirm_password_required')),
                },
                ({ getFieldValue }) => ({
                  validator(_, value) {
                    ...
                    return Promise.reject(
                      new Error(t(getKey('passwords_do_not_match')))
                    );
                  },
                }),
              ]}
        ...
        <Form.Item className={cn('!mb-0')}>
          <Button
            ...
            className={cn(
              '!rounded-lg !text-base !font-medium !text-blackSoft'
            )}
          >
