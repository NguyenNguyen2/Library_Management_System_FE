...
interface LibraryContextType {
  ...
  renewTransaction: (transactionId: string) => boolean | 'reserved';
...
export function LibraryProvider({ children }: { children: React.ReactNode }) {
  ...
  const renewTransaction = (transactionId: string): boolean | 'reserved' => {
    const transaction = transactions.find(t => t.id === transactionId);
    if (!transaction || transaction.renewCount >= transaction.maxRenewals) return false;

    // Block renewal if someone else has reserved this book
    const hasReservation = reservations.some(
      r => r.bookId === transaction.bookId &&
           r.readerId !== transaction.readerId &&
